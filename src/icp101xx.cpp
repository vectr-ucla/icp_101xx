...
bool ICP101xx::begin(TwoWire *wire) {
  ...
  uint8_t addr_otp_cmd[5] = {(ICP_CMD_SET_ADDR >> 8) & 0xff,
                             ICP_CMD_SET_ADDR & 0xff, 0x00, 0x66, 0x9c};
  uint8_t otp_buf[3];
...
bool ICP101xx::dataReady(void) {
  ...
  uint32_t L_res_buf3 =
      res_buf[3]; // expand result bytes to 32bit to fix issues on 8-bit MCUs
...
float ICP101xx::getTemperatureC(void) { return _temperature_C; }

float ICP101xx::getTemperatureF(void) { return _temperature_C * 1.8 + 32; }

float ICP101xx::getPressurePa(void) { return _pressure_Pa; }
...
void ICP101xx::_calculate(void) {
  ...
  float s2 =
      _offst_factor * _scal[3] + (float)(_scal[1] * t * t) * _quadr_factor;
  ...
  float c = (s1 * s2 * (_pcal[0] - _pcal[1]) + s2 * s3 * (_pcal[1] - _pcal[2]) +
            ...
            (s3 * (_pcal[0] - _pcal[1]) + s1 * (_pcal[1] - _pcal[2]) +
             s2 * (_pcal[2] - _pcal[0]));
  float a =
      (_pcal[0] * s1 - _pcal[1] * s2 - (_pcal[1] - _pcal[0]) * c) / (s1 - s2);
